...
router.post("/cadastro", async (req, res) => {
  ...
  try {
    ...
    // Retorna o id para o frontend salvar no AsyncStorage
    res.status(201).json({ token, usuario: { id: user.id, nome, email } });
  } catch (err) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    // Retorna o id para o frontend salvar no AsyncStorage
    res.json({ token, usuario: { id: user.id, nome: user.nome, email } });
  } catch (err) {
    res.status(500).json({ erro: "Erro no servidor" });
  }
});
